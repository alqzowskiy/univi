...
class UniversalCostCalculator:
    ...
    def calculate_costs(self, university: str) -> dict:
        """Calculate costs for any university worldwide using AI."""
        try:
            # Get accurate costs using Gemini
            prompt = f"""Provide EXACT ACCURATE cost breakdown for {university}.
...
def calculate_costs():
    ...
    try:
        ...
        # Get cost data
        cost_prompt = f"""Return exact verified costs for {university} in USD as JSON:
        {{
            "monthly_costs": {{
